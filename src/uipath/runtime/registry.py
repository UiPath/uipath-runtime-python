...
class UiPathRuntimeFactoryRegistry:
    ...
    def get(
        ...
        Args:
            name: Optional factory name. When supplied, returns the
                registered factory for that name.
            search_path: Path to search for config files when ``name`` is
                not supplied. Auto-detection walks the registered
                factories in reverse-registration order and picks the
                first one whose declared config file exists at
                ``search_path``.
            context: :class:`UiPathRuntimeContext` to pass to the factory
                constructor.

        Returns:
            The registered :class:`UiPathRuntimeFactoryProtocol`
            instance. Cross-cutting concerns (governance, audit, …) are
            composed by the host into the decorator chain — they are
