...
# Canonical name of the governance wrapper. Used both as the only
# allowed registration name and as the wrapper subject to the feature
# flag gate in wrap_runtime().
GOVERNANCE_WRAPPER_NAME = "governance"

# Allowed wrapper names - only these can be registered.
# This prevents malicious packages from injecting wrappers.
ALLOWED_WRAPPERS: frozenset[str] = frozenset({GOVERNANCE_WRAPPER_NAME})

# Allowed packages that can register wrappers via entry points.
# Entry points from other packages are ignored.
ALLOWED_WRAPPER_PACKAGES: frozenset[str] = frozenset({"uipath-governance"})

# Feature flag controlling whether the governance wrapper is applied.
# Default-off semantics: when the flag is missing, unresolved, or the
# FeatureFlags module is not importable, governance is NOT applied. Only
# an explicit truthy value on this flag enables governance.
...
def _is_governance_enabled() -> bool:
    ...
    Delegates resolution to uipath-core's ``FeatureFlags.is_flag_enabled``.
    Core's resolution order: programmatic configuration (typically
    populated from gitops at startup) > ``UIPATH_FEATURE_<name>``
    environment variable > ``default``. The ``default=False`` keeps the
    gate closed when the flag is not set anywhere.
    """
    # Imported lazily so this module stays importable even if the
    # uipath-core install is incomplete at evaluation time.
...
class UiPathRuntimeWrapperRegistry:
    ...
    def _get_package_name_from_entry_point(self, ep: Any) -> str | None:
        """Extract the distribution name from an entry point.

        Returns ``None`` when the entry point has no associated distribution
        (e.g., orphaned metadata). On Python 3.10+ ``ep.dist`` is reliably
        populated for entry points discovered via ``importlib.metadata``.
        """
        try:
            dist = ep.dist
    ...
    def load_entry_points(self) -> None:
        ...
        for ep in eps:
            ...
            try:
                ...
                # Distinct from the generic catch below — surfaces
                # ALLOWED_WRAPPERS violations from register() at a different
                # log level so operators can tell config error from runtime
                # error.
    ...
    async def wrap_runtime(
        ...
        # Evaluate the governance gate once before the loop so we don't
        # re-resolve the feature flag on every iteration.
        governance_enabled = _is_governance_enabled()

        wrapped = runtime
        for name, (wrapper, priority) in sorted_wrappers:
            if name == GOVERNANCE_WRAPPER_NAME and not governance_enabled:
                logger.info(
