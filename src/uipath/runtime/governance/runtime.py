...
- resolve a policy pack for the current run — any provider, any code
  path, any wire format,
- produce a :class:`~uipath.runtime.governance.native.PolicyIndex`
  from that policy; the runtime is format-agnostic and only accepts a
  compiled index,
- skip wrapping entirely when the resolved enforcement mode is
...
def _governance_root_span(agent_name: str, runtime_id: str) -> Iterator[None]:
    ...
    with tracer.start_as_current_span("uipath.governance.run") as span:
        ...
        span.set_attribute("uipath_governance.source", "governance-runtime-python")
        yield
...
class UiPathGovernedRuntime:
    ...
    ``execute`` / ``stream`` / ``get_schema`` forward to the delegate.
    ``dispose`` is a no-op — the caller owns the delegate (and every
    other host-managed resource) and is responsible for tearing them
    down. The method is kept only to satisfy
    :class:`UiPathDisposableProtocol` so the wrapper stays
    structurally substitutable for any :class:`UiPathRuntimeProtocol`.
    ...
    async def dispose(self) -> None:
        """No-op — the caller owns delegate + host-resource lifecycle.

        The wrapper deliberately does not forward to
        ``self._delegate.dispose()`` (the caller instantiated the
        delegate and is responsible for disposing it) and does not
        invoke any host-supplied cleanup hook (the host runs its own
        teardown on the same code path). Kept as a method purely to
        satisfy :class:`UiPathDisposableProtocol` — the wrapper stays
        structurally substitutable for any ``UiPathRuntimeProtocol``.
        """
