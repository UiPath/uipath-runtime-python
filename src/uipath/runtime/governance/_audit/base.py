...
class AuditSink(ABC):
    ...
    def flush(self) -> None:  # noqa: B027 — optional hook; subclasses override
        """Flush any buffered events.

        Override if sink buffers events before writing.
        """

    def close(self) -> None:  # noqa: B027 — optional hook; subclasses override
        """Clean up resources.
...
class AuditManager:
    ...
    def emit(self, event: AuditEvent) -> None:
        ...
        for sink in sinks:
            ...
            except Exception as exc:  # noqa: BLE001 — swallow to guard the agent
                self._record_sink_failure(sink.name, exc)

    def _record_sink_failure(self, sink_name: str, exc: Exception) -> None:
        """Bump the circuit-breaker counter for ``sink_name`` and log.

        Extracted from :meth:`emit` to keep that method's cognitive
        complexity below the linter's threshold. The lock-guarded
        counter mutation and the branch between "still under threshold"
        vs "just tripped" logging live here as a single responsibility.
        """
        with self._sinks_lock:
            fails = self._sink_failures.get(sink_name, 0) + 1
            self._sink_failures[sink_name] = fails
            tripped_now = fails >= self._SINK_FAILURE_THRESHOLD
            if tripped_now:
                self._tripped_sinks.add(sink_name)
        if tripped_now:
            # ``logger.exception`` captures the traceback via
            # ``exc_info`` — no need to interpolate ``exc`` into
            # the message (Sonar S8572).
            logger.exception(
                "Audit sink '%s' tripped after %d consecutive failures; "
                "will be skipped for the rest of this process.",
                sink_name,
                fails,
            )
        else:
