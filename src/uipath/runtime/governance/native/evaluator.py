...
_INCIDENT_PATTERNS: dict[str, list[re.Pattern[str]]] = {
    ...
    "tool_failure": [
        ...
        re.compile(r"(?i)\b(timed?\s*out)\b"),
    ],
    "auth_failure": [
...
class GovernanceEvaluator:
        ...
        *,
        ...
        Args:
            policy_index: The compiled :class:`PolicyIndex` to evaluate.
                ...
                — built by the caller from whatever policy source they
                own; the evaluator only consumes the compiled index.
    ...
    def _dispatch_compensation(self, audit: AuditRecord, context: CheckContext) -> None:
        ...
        try:
            ...
            logger.warning("Failed to dispatch compensating governance call: %s", exc)
    ...
    def _evaluate_rule(self, rule: Rule, context: CheckContext) -> RuleEvaluation:
        ...
        for check in rule.checks:
            ...
            if matched:
                ...
                elif check.action == Action.ESCALATE and matched_action != Action.DENY:
                    matched_action = Action.ESCALATE
                elif check.action == Action.AUDIT and matched_action == Action.ALLOW:
    ...
    def _check_commitment_concern(text: str, params: Any) -> bool:
        ...
        amount_match = require_amount and bool(_COMMITMENT_AMOUNT_FALLBACK.search(text))
