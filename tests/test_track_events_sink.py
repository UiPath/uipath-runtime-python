...
@pytest.fixture
def sink(
    capture: _Capture, metadata: GovernanceRuntimeMetadata
) -> TrackEventAuditSink:
...
def test_rejects_disabled_mode_rule_evaluation(sink: TrackEventAuditSink) -> None:
    ...
    assert (
        sink.accepts(_rule_event(matched=True, mode=EnforcementMode.DISABLED))
        is False
    )
...
def test_hook_summary_duration_ms(
    sink: TrackEventAuditSink, capture: _Capture
) -> None:
...
def test_hook_summary_mode_string(
    sink: TrackEventAuditSink, capture: _Capture
) -> None:
