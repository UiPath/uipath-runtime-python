...
def test_audit_mode_emits_per_rule_and_summary_events(audit_setup: Any) -> None:
    ...
    rule_events = [e for e in sink.events if e.event_type == EventType.RULE_EVALUATION]
    summary_events = [e for e in sink.events if e.event_type == EventType.HOOK_END]
...
def test_sink_failure_does_not_propagate_or_block_evaluation(
    ...
    assert any(e.event_type == EventType.RULE_EVALUATION for e in capturing_sink.events)
