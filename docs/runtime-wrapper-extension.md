...
The governance wrapper is gated by the `EnablePythonGovernanceChecker` feature flag (read via `uipath.core.feature_flags.FeatureFlags`). Default-off: when the flag is not set anywhere, the governance wrapper is **not** applied — the inner runtime is returned unwrapped.

Resolution order (delegated to uipath-core):

1. Programmatic configuration via `FeatureFlags.configure_flags({...})` (typically populated from gitops at startup)
2. `UIPATH_FEATURE_EnablePythonGovernanceChecker` environment variable
3. `default=False`

This means a strict-opt-in posture: governance only runs in environments where the flag has been explicitly enabled.
...
Note: a wrapper being *registered* doesn't guarantee it's *applied* — the feature flag still gates the actual application inside `wrap_runtime()`.
